...
def calibrate_image(raw_image, master_bias, master_dark, master_flat, exposure_time=1.0, dark_exposure=1.0):
    ...
    # Subtract scaled dark (thermal noise)
...
def align_image(target_image, reference_image):
    ...
    relative to reference_image by matching stellar asterisms (triangles).
...
def generate_master_reference(burn_in_frames):
    ...
    This completely removes the pipeline's dependency on historical catalogs, 
    and mathematically erases transient cosmic rays or satellite streaks.
    ...
    for i, frame in enumerate(burn_in_frames[1:]):
        try:
            ...
            # If a single burn-in frame fails to align (e.g. a cloud passed over), 
            # we can safely skip it and build the reference from the remaining frames.
            continue
